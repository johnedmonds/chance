...
mod tests {
    ...
    fn test_permutations() {
        assert_eq!(
            vec![vec![2, 1], vec![1, 2]],
            permutations(vec![1, 2]).collect::<Vec<Vec<i32>>>()
        );
    }
    ...
    fn test_power_set() {
        assert_eq!(
            ...
            power_set(vec![1, 2]).collect::<Vec<Vec<i32>>>()
        );
    }
        ...
        assert_eq!(
            vec![
                Operation::Operation(
                    2 as i32,
                    Operator::ADD,
                    Box::new(Operation::SingleOperand(1 as i32))
                ),
                Operation::Operation(
                    1 as i32,
                    Operator::ADD,
                    Box::new(Operation::SingleOperand(2 as i32))
                )
            ],
            find_operations_for_value(vec![1, 2], 3).collect::<Vec<Operation<i32>>>()
        );
    }
    ...
    fn test_filter_associative() {
        assert_eq!(
            ...
            process_associative_operation_filter(
                vec![
                    Operation::Operation(
                        2 as i32,
                        Operator::ADD,
